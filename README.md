...
- **多支付方式**：**微信 / 支付宝 / USDT（加密货币）** 自由选择，买家选套餐后弹选择，发对应收款码/地址
- **多支付方式**：**微信 / 支付宝 / USDT（加密货币）** 自由选择，每种支持多张收款码随机轮换
- **取消订单**：收到付款信息后可点「❌ 取消订单」，立即作废并重新选套餐
...
- **📦 订单管理**：待审核（带**开通/取消**按钮的分页列表）/已处理/收款流水；买家端新增「📋 我的订单」自助查看订单状态/取消订单
- **🔗 上游池管理**：无限添加上游，按用户分配或合并全部节点
- **📡 节点管理**：查看/单个/批量禁用节点（支持 `1,3,5` 或 `all`）
- **⚙️ 系统设置**（⚙️ 更多功能内）：上游/支付方式(微信支付宝USDT+汇率)/价格/时长/公告/客服（全按钮化）
...
### 3. 配置支付方式（关键）
```
⚙️ 更多功能 → 🧰 系统设置 → 💳 支付方式 → 配置微信/支付宝收款码 或 USDT 地址/汇率
```
至少配置一种，买家才能购买。用 USDT 收款时建议顺手设置汇率（默认 1U = 7.2 元）。
...
| 用户统计 | ⚙️ 更多功能 →「📈 用户统计」 | 正常/过期/禁用分布 + 7天内到期 |
| 即将到期 | 主菜单「⏳ 即将到期」 | 列出 7 天内到期的用户 |
| 导出名单 | ⚙️ 更多功能 →「📤 导出名单」 | 导出全部用户信息（UID/ChatID/状态/到期/备注） |
...
| 删除未使用 | 子菜单「🗑️ 删除未使用卡密」 | **一键删除所有未使用卡密**（清库存，已用/禁用保留） |
...
> ⚙️ 更多功能 → 🧰 系统设置 →「💳 支付方式」，买家购买时可选择已配置的支付方式（微信/支付宝/USDT）。

| 功能 | 操作 |
|------|------|
| 配置微信收款码 | 「📱 配置微信收款码」→ 发图（可多张）→ `/done` |
| 配置支付宝收款码 | 「🧧 配置支付宝收款码」→ 发图（可多张）→ `/done` |
| 配置 USDT | 「🪙 配置 USDT 地址」→ 输入 `地址 网络` |
| 设置 USDT 汇率 | 「💱 设置 USDT 汇率」→ 输入 `1 USDT = N 元`（默认 7.2） |
| 查看/删除 | 「📋 查看/删除已配置」→ 回复「删微信 序号」「删支付宝 序号」或「清空USDT」 |

> **USDT 汇率**：买家选 USDT 付款时，套餐人民币金额按汇率自动折算成 USDT 显示（如 20元 ≈ 2.78 U）。汇率浮动请按行情定期更新，实时生效。
> 每种支付方式支持多张收款码，买家选择后**随机展示**其中一张。至少配置一种支付方式，买家才能下单。
...
|------|------|
| 上游池管理 | 查看/管理全部上游 |
| **支付方式** | 配置 微信/支付宝 收款码 + USDT 地址/汇率，买家可选支付方式 |
| 发布公告 | 输入公告内容（买家前台 Bot 发 /start 可见） |
| 设置价格 | 输入价格内容（如 `30元/月`） |
| 设置时长 | 选择或输入默认续费时长 |
| 设置客服 | 输入客服联系方式（`@用户名` 或 `https://t.me/用户名`） |

**支付方式配置**（⚙️ 更多功能 → 🧰 系统设置 → 💳 支付方式）：
- **📱 配置微信收款码**：发微信收款码图片（可多张，发完 /done）
- **🧧 配置支付宝收款码**：发支付宝收款码图片（可多张，发完 /done）
- **🪙 配置 USDT 地址**：输入 `地址 网络`（如 `TXYZ1234 TRC20`）
- **💱 设置 USDT 汇率**：输入 `1 USDT = N 元`，买家 USDT 付款按此汇率折算套餐金额
- **📋 查看/删除已配置**：查看列表，回复「删微信 序号」/「删支付宝 序号」删除，或「清空USDT」
- 买家购买时只看到**已配置**的支付方式；未配置任何方式时提示管理员先配置

### 系统概览与操作日志

- **📊 系统概览**（主菜单）：一屏看全：今日经营（新购/续费/卡密/流水金额）+ 用户分布 + 待审订单 + 卡密库存 + 套餐 + 支付方式 + 上游池 + 合并模式
- **📜 操作日志**（⚙️ 更多功能 →「📜 操作日志」）：最近 20 条关键操作记录（发货/开卡/调整/删除/套餐/分销等）
...
| 点「🔍 查询订阅」 | 查看自己的订阅状态 |
| 点「📋 我的订单」 | 自助查看进行中订单（待付款/待审核），可一键取消 |
| 点「🎫 兑换卡密」+ 输入卡密 | 自助兑换 |
| 点「🎁 优惠券」+ 输入券码 | 优惠券兑换 |
| 点「❓ 常见问题」 | 常见问题自助解答 |
| 点「📞 联系客服」 | 一键联系客服 |
...
1. 打开前台 Bot → 点【🛒 购买套餐】
2. 选择套餐（只显示在售套餐）
3. **选择支付方式**（微信/支付宝/USDT，只显示已配置的）
4. 扫码付款（或 USDT 转账）→ 发送转账截图
5. 管理员确认 → 秒开通
6. **不买了/选错了？** 付款信息下方点「❌ 取消订单」，立即作废并重新选
7. **随时查进度？** 点【📋 我的订单】可看到每笔订单状态（🟡 待付款 / ⏳ 待审核）并一键取消
...
```
⚙️ 更多功能 → 🧰 系统设置（部署时配置一次即可）
├── 🔗 上游池     → /addurl 添加所有上游线路
├── 💳 支付方式   → 配置微信/支付宝收款码 + USDT 地址/汇率
├── 💰 价格       → 点按钮设置套餐价格
├── 📅 时长       → 点按钮设置默认时长
├── 📢 公告       → 发布店铺公告
└── 📞 客服       → /service @客服用户名
