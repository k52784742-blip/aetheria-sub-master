...
- **⚙️ 系统设置**：上游/支付方式(微信支付宝USDT)/价格/时长/公告/客服（全按钮化）
...
### 3. 配置支付方式（关键）
```
⚙️ 系统设置 → 💳 支付方式 → 配置微信/支付宝收款码 或 USDT 地址
```
至少配置一种，买家才能购买。
...
> 系统设置 →「💳 支付方式」，买家购买时可选择已配置的支付方式（微信/支付宝/USDT）。
...
|------|------|
| 上游池管理 | 查看/管理全部上游 |
| **支付方式** | 配置 微信/支付宝 收款码 + USDT 地址，买家可选支付方式 |
...
- **📊 系统概览**：一屏看全：今日经营（新购/续费/卡密/流水金额）+ 用户分布 + 待审订单 + 卡密库存 + 套餐 + 支付方式 + 上游池 + 合并模式
