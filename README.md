...
### 2026-09-04 全功能闭环人性化完善
- **禁用/启用/删除用户 → 通知买家**：管理员操作后买家立即收到「服务暂停/已恢复/订阅已取消」通知，不再"莫名其妙不能用"
- **付款凭证新增【❌ 拒绝】按钮**：凭证无效时管理员可拒绝，买家收到原因说明并重新提交（带幂等防重复点击）
- **群发通知二次确认**：输入内容后先预览（显示将发送给多少位用户）再确认发送，防止误发给所有人
- **订阅 expire 头立即更新**：调整时长/续费/卡密兑换/优惠券兑换/撤销操作后统一清理全部缓存变体（普通/legacy/yaml），客户端订阅到期时间不再延迟 2 小时
- **禁用/启用用户立即生效**：操作后清缓存，订阅立刻停止/恢复服务
- **分配上游/恢复自动分配清全部缓存**：修复只清单一缓存键导致 legacy/yaml 订阅仍用旧线路的问题
