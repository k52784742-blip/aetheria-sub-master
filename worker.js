...
async function sendDailyReport(env) {
  try {
    ...
    // 用 UTC 时区确定"昨日"（Cron 在 UTC 0 点触发，此时本地时区可能跨天）
...
async function handleBuyerPortal(uid, request, env) {
  ...
  if (isBrowser) {
    ...
    const plan = escHTML(user.plan || "标准套餐");
    ...
    // 公告展示
    const notice = escHTML((await env.SUB_STORE.get("notice_content")) || "");
...
<body>
  ...
  if (mergeMode) {
    ...
    // 宽松 base64 解码：去除空白字符后尝试，解码结果必须含 "://" 才采用
    try {
      let b64 = decoded.replace(/\s+/g, "");
      const pad = 4 - (b64.length % 4);
      if (pad < 4) b64 += "=".repeat(pad);
      const decodedStr = new TextDecoder().decode(Uint8Array.from(atob(b64), c => c.charCodeAt(0)));
      if (decodedStr.includes("://")) decoded = decodedStr;
    } catch (e) {}
  ...
  // 空结果保护：无可用节点时给出明确提示，避免返回空订阅导致客户端导入失败
  if (processedNodes.length === 0) {
    return new Response("当前无可用节点，请联系管理员", { status: 502 });
  }
  ...
  // 分块编码，避免 String.fromCharCode(...大数组) 栈溢出导致订阅生成失败
  const finalBase64 = utf8ToBase64(finalOutput);
  ...
  for (const k of userKeys) {
    ...
    for (const remindDay of REMINDER_DAYS) {
      if (remainDays === remindDay) {
        ...
        // 记录已通知（k 是完整 key 字符串，如 "user_3422"；不能写 k.name）
  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (data.startsWith("approve_renew_")) {
        ...
        // 幂等保护：同一通知消息只处理一次，防止重复点击重复加天数/重复通知
        const rProcessedKey = `processed_${cb.message.message_id}`;
        const rAlreadyProcessed = await env.SUB_STORE.get(rProcessedKey);
        if (rAlreadyProcessed) {
          ...
          if (!rUserStr) {
            ...
            await env.SUB_STORE.put(rProcessedKey, JSON.stringify({ uid: rUid, time: Date.now() }), { expirationTtl: 86400 });
      ...
      // 回调格式：approve_{chatId}_{orderId}（orderId 可选，兼容旧按钮）
        ...
        if (alreadyProcessed) {
          ...
          // 更新通知消息：标记已处理，仅保留撤销按钮（移除确认/稍后按钮，防止重复点击）
          try {
            const markup = {
              inline_keyboard: [
                [
...
// ===== 通用工具：UTF-8 安全的 base64 编码 =====
// 分块处理，避免 String.fromCharCode(...大数组) 栈溢出；
// 同时正确处理中文/emoji 等多字节字符（不能直接 btoa 字符串）
function utf8ToBase64(str) {
  const bytes = new TextEncoder().encode(str);
  let bin = "";
  const chunkSize = 0x8000; // 32768 字节/块，远低于调用栈上限
...
// ===== 通用工具：Markdown 安全转义 =====
// MarkdownV1 中 _ * [ ] ` 等字符会破坏解析，导致 sendMessage 400。
// 买家昵称/用户名/备注等用户可控文本一律先转义再拼接进消息。
function escMD(text) {
  return String(text || "").replace(/([_*\[\]`])/g, "\\$1");
}

// ===== 通用工具：HTML 安全转义 =====
// 公告/套餐名等用户可控字段渲染进 HTML 时转义，防止 XSS
...
async function sendTGText(token, chatId, text) {
  ...
  if (!res.ok) {
    // 降级重试：去掉 parse_mode（特殊字符如 _ * [ ] 不会破坏纯文本）
    try {
...
// 发送带常驻底部菜单的文本（Markdown 解析失败时自动降级纯文本重发）
...
// 编辑已有消息（Markdown 解析失败时自动降级纯文本重发）
